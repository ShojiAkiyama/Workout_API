...
async def post(db_session: DatabaseDependency,
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail=f'Já existe um atleta cadastrado com o cpf: {cpf_numero}'
        )
...
async def query(db_session: DatabaseDependency) -> Page[AtletaOut]:
    atleta = select(AtletaModel)
    
    return await paginate(db_session, atleta)
...
async def query(id: UUID4, nome: str, cpf: str, db_session: DatabaseDependency) -> AtletaOut:
    atleta: AtletaOut = (await db_session.execute(select(AtletaModel).filter_by(id=id, nome=nome, cpf=cpf))).scalars().first()
